 ...
 * raylib 5.5
 *
 *   Example contributed by Agnis Aldins (@nezvers) and reviewed by Ramon
 * Santamaria (@raysan5)
 *
...
int main(void) {
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    if (player.isGrounded && ((forward != 0) || (sideway != 0))) {
      ...
      walkLerp = Lerp(walkLerp, 1.0f, 30.0f * delta);
      camera.fovy = Lerp(camera.fovy, 65.0f, 5.0f * delta);
    } else {
...
static void UpdateCameraFPS(Camera *camera) {
  ...
  // Left and right Using Y axis as a referencial for horizontal rotation to collect your vision direction
  ...
  if (-(lookRotation.y) > maxAngleUp) {
    lookRotation.y =
        -maxAngleUp; // Avoid to look backwards by looking to the ground
  }
